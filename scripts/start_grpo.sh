...
OUTPUT_DIR="${ROOT_DIR}/output/grpo_rdr_only_v2_ft"
...
echo "========================================="
echo "GRPO 消融 - 仅 RDR 奖励 (v2 直接质量评分)"
echo "========================================="
echo "  策略模型    : Qwen3-8B"
echo "  RDR 奖励    : ${RDR_CKPT}"
echo "  AS/DRT      : 禁用"
echo "  训练卡      : GPU 1-7（7卡，num_generations=7）"
echo "========================================="

# ── Step 1: 检查 RDR checkpoint ──────────────────────────────
echo "[Step 1] 检查 RDR checkpoint..."
if [ ! -f "${RDR_CKPT}/model.pt" ]; then
    echo "  [ERROR] RDR checkpoint 不存在: ${RDR_CKPT}/model.pt"
    exit 1
fi
echo "  ✅ RDR: ${RDR_CKPT}/model.pt ($(du -sh ${RDR_CKPT}/model.pt | cut -f1))"
...
# ── Step 4: 启动 RDR 奖励服务器（GPU 0 独占）────────────────
echo "[Step 4] 启动 RDR 奖励服务器 (port=${REWARD_PORT}, GPU=0)..."
REWARD_LOG="${LOG_DIR}/reward_server_rdr_only_v2_${TIMESTAMP}.log"
cd "${CHAPTERRL}"
...
TRAIN_LOG="${LOG_DIR}/grpo_rdr_only_v2_${TIMESTAMP}.log"
cd "${SWIFT_DIR}"
...
CUDA_VISIBLE_DEVICES=1,2,3,4,5,6,7 nohup swift rlhf \
    ...
    --report_to wandb \
    --run_name "grpo_rdr_only_v2_ft_${TIMESTAMP}" \
    > "${TRAIN_LOG}" 2>&1 &
